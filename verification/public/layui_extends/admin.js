...
layui.define(['view', 'layer'],function(exports){
  var $ = layui.jquery
  ,laytpl = layui.laytpl
  ...
  ,admin = {
    ...
    ,moduleToken: function(...args){
      let gameArr = layui.sessionData(layui.setter.tableName, 'gameSelect');
      let setHeader = {
        Authorization: layui.data(layui.setter.tableName, 'Authorization'),
        Game: !gameArr? '' : gameArr['id'],
        Platform: !gameArr ? '' : gameArr['platformId'],
        Partner: !gameArr ? '' : gameArr['partnerId'],
      };
      for (let val of args) {
        !layui[val] || layui[val].set({
          headers: setHeader
        });
      }
    },
    ...
    removeAllCookie: function () {
      ...
      const domainArr = location.host.split('.').reverse()
      if (keys) {
        for (let i = keys.length; i--;) {
          let domain = ''
          domainArr.forEach((item, index) => {
            domain = index  === 0 ? item : `${item}.${domain}`
            this.removeCookie(keys[i], "", "")
            this.removeCookie(keys[i], "", domain)
            this.removeCookie(keys[i], "/", "")
            this.removeCookie(keys[i], "/", domain)
          })
        }
  ...
  var events = admin.events = {
    ...
    //刷新
    ,refresh: function(){
      layui.element.done && layui.element.done()
      layui.element.done = null
      layui.index.render();
    }
  ...
  //页面跳转
  $body.on('click', '*[lay-href]', admin.debounce(function(){
    var othis = $(this)
    ,href = othis.attr('lay-href')
    ,router = layui.router();

    // 补充： 左侧菜单切换，向外暴露菜单数据
    admin.menu = othis.data()
  ...
  //   const elem = e.target;
  //   if (elem.nodeName === 'BUTTON' && RegExp('提交|确定', 'g').test(elem.innerText)){
  //     elem.disabled = true
  //     setTimeout(()=>{
  //       elem.disabled = false
  //     }, 1500)
